...
//     const totalAmount = faker.number.int({ min: 5000, max: 10000 }) + Math.random();
//     const paidAmount = faker.number.int({ min: 0, max: Math.floor(totalAmount) }) + Math.random();
//     const balance = Math.max(0, totalAmount - paidAmount); // Calculate correct balance
...
//     const classSubjects = faker.helpers.arrayElements(subjects, { min: 5, max: 8 });
...
//       // Create 2-3 lessons per subject per week
...
//         // Create attendance records for this lesson
...
//           // Create results for the assignment
//           for (const student of classStudents) {
