...
export const ReportCard = ({ 
  ...
  return (
    <div id="report-card" className="bg-white border-0 rounded-lg shadow-xl overflow-hidden max-w-4xl mx-auto print:scale-90">
      ...
      {/* Student Information - more compact */}
      ...
      {/* Academic Performance - more compact table */}
      <div className="p-4">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th scope="col" className="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Score</th>
                ...
                {subjectScores.map((subject, index) => {
                  ...
                  return (
                    <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      <td className="px-3 py-2 whitespace-nowrap text-xs font-medium text-gray-900">{subject.subject}</td>
                      <td className="px-3 py-2 whitespace-nowrap text-xs text-center font-medium">{subject.score.toFixed(1)}</td>
                      <td className={`px-3 py-2 whitespace-nowrap text-xs text-center font-bold ${getGradeColor(grade)}`}>{grade}</td>
              ...
              <tfoot className="bg-gray-50">
                <tr>
                  <td className="px-3 py-2 whitespace-nowrap text-xs font-medium text-gray-900">Overall Average</td>
                  <td className="px-3 py-2 whitespace-nowrap text-xs text-center font-bold">{averageScore.toFixed(1)}</td>
                  <td className={`px-3 py-2 whitespace-nowrap text-center text-xs font-bold ${getGradeColor(getGrade(averageScore))}`}>
                    {getGrade(averageScore)}
                  </td>
                  <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-500">
                    {averageScore >= 80 
